    ...
    >
        <template #header>
            <div class="flex justify-between items-center">
                ...
                <Button label="Add Design" icon="pi pi-plus" @click="$router.push({ name: 'config-general-frame-create' })" />
            </div>
        </template>
        ...
        <Column header="Actions" style="width: 10%">
            <template #body="{ data }">
                <div class="flex gap-2">
                    ...
                    <Button v-if="data.deleted_at === null" icon="pi pi-pencil" class="p-button-text" @click="editFrame(data.id)" :disabled="data.title === 'No Design' && authStore.user.role !== 'super_admin'" v-tooltip="'Edit Design'" />
