    ...
    >
        <template #header>
            <div class="flex justify-between items-center">
                <h3 class="text-lg font-semibold">Frame Management</h3>
                <Button label="Add Frame" icon="pi pi-plus" @click="$router.push({ name: 'config-general-frame-create' })" />
            </div>
        </template>
        ...
        <Column header="Actions" style="width: 10%">
            <template #body="{ data }">
                <div class="flex gap-2">
                    ...
                    <Button v-if="data.deleted_at === null" icon="pi pi-pencil" class="p-button-text" @click="editFrame(data.id)" :disabled="data.title === 'No Frame' && authStore.user.role !== 'super_admin'" v-tooltip="'Edit Frame'" />
                    ...
                    <Button
                        ...
                        :disabled="data.title === 'No Frame' && authStore.user.role !== 'super_admin'"
                        v-tooltip="data.deleted_at === null ? 'Delete Frame' : 'Restore Frame'"
                    />
                </div>
            </template>
        </Column>
